...
/// <summary>
/// PDF document generator for Nota de Débito Electrónica (ND - tipo 02)
/// Requires reference to original document
/// </summary>
public class NotaDebitoPdfDocument : BasePdfDocument
{
    // Override colors for ND (red theme)
    private new static readonly string PrimaryColor = "#c0392b";
    private new static readonly string SecondaryColor = "#e74c3c";

    public NotaDebitoPdfDocument(Documento documento) : base(documento)
    {
    }
    ...
    public override DocumentMetadata GetMetadata()
    {
        ...
        {
            Title = $"Nota de Débito Electrónica - {Documento.NumeroConsecutivo}",
            Author = Documento.Empresa?.RazonSocial ?? "Sistema de Facturación",
            Subject = "Nota de Débito Electrónica Costa Rica - Hacienda v4.4",
            Keywords = "nota de débito, electrónica, costa rica, hacienda",
    ...
    private void ComposePageHeader(IContainer container)
    {
        ...
        {
            col.Item().Element(ComposeHeaderND);
            col.Item().Height(10);
            col.Item().Element(ComposeClaveSection);
        });
    }

    private void ComposeHeaderND(IContainer container)
    {
        container.Row(row =>
        {
            ...
            // Right side - Document type with red theme
            row.RelativeItem(1).Border(2).BorderColor(PrimaryColor).Background("#fdf2f2").Padding(10).Column(col =>
            {
                col.Item().AlignCenter().Text("NOTA DE DÉBITO")
    ...
    private void ComposePageContent(IContainer container)
    {
        ...
        {
            // Important: Reference section first for ND
            ...
            col.Item().Row(row =>
            {
                ...
                // Right column - Totals
                row.ConstantItem(220).Element(ComposeTotalsND);
            });
    ...
    /// <summary>
    /// Compose highlighted reference section for ND
    /// </summary>
    private void ComposeReferenciasDestacado(IContainer container)
    {
        ...
        container.Border(2).BorderColor(PrimaryColor).Background("#fdf2f2").Column(col =>
        {
    ...
    /// <summary>
    /// Compose totals section with ND styling
    /// </summary>
    private void ComposeTotalsND(IContainer container)
    {
        container.Border(2).BorderColor(PrimaryColor).Column(col =>
        {
            col.Item().Background(PrimaryColor).Padding(5)
                .Text("RESUMEN - NOTA DE DÉBITO").FontSize(NormalFontSize).Bold().FontColor(Colors.White);

            col.Item().Padding(8).Column(content =>
            {
                if (Documento.TotalGravado > 0)
                    ComposeTotalRowND(content, "Total Gravado:", Documento.TotalGravado);

                if (Documento.TotalExento > 0)
                    ComposeTotalRowND(content, "Total Exento:", Documento.TotalExento);

                if (Documento.TotalExonerado > 0)
                    ComposeTotalRowND(content, "Total Exonerado:", Documento.TotalExonerado);

                ComposeTotalRowND(content, "Subtotal:", Documento.Subtotal);

                if (Documento.TotalDescuentos > 0)
                    ComposeTotalRowND(content, "Total Descuentos:", Documento.TotalDescuentos);

                if (Documento.TotalImpuestos > 0)
                    ComposeTotalRowND(content, "Total Impuestos:", Documento.TotalImpuestos);

                if (Documento.TotalOtrosCargos > 0)
                    ComposeTotalRowND(content, "Otros Cargos:", Documento.TotalOtrosCargos);

                content.Item().Height(5);
                content.Item().BorderBottom(2).BorderColor(PrimaryColor);
                content.Item().Height(5);

                // Emphasize this is a debit (additional charge - TotalComprobante = TotalVenta + TotalOtrosCargos - IVADevuelto)
                var totalComprobante = Documento.TotalVenta + Documento.TotalOtrosCargos - (Documento.IVADevuelto ?? 0);

                content.Item().Row(row =>
                {
                    row.RelativeItem().AlignRight().Text("TOTAL A DEBITAR:")
                        .FontSize(SubtitleFontSize).Bold().FontColor(PrimaryColor);
                    row.ConstantItem(10);
    ...
    private void ComposeTotalRowND(ColumnDescriptor content, string label, decimal amount)
    {
        content.Item().Row(row =>
        {
