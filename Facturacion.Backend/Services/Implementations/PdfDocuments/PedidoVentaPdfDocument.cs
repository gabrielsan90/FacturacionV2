using Facturacion.Backend.Helpers;
using Facturacion.Shared.Entities;
using Facturacion.Shared.Enums;
using QuestPDF.Fluent;
using QuestPDF.Helpers;
using QuestPDF.Infrastructure;

namespace Facturacion.Backend.Services.Implementations.PdfDocuments;

/// <summary>
/// PDF document generator for Pedido de Venta (Sales Order)
/// </summary>
public class PedidoVentaPdfDocument : IDocument
{
    private readonly PedidoVenta _pedido;
    private readonly TipoMoneda _moneda;

    private static readonly string PrimaryColor = "#1a5276";
    private static readonly string SecondaryColor = "#2980b9";
    private static readonly string HeaderBgColor = "#ecf0f1";
    private static readonly string AlternateRowColor = "#f8f9fa";
    private static readonly string BorderColor = "#dee2e6";
    private static readonly string TextColor = "#2c3e50";
    private static readonly string LightTextColor = "#7f8c8d";

    private const float TitleFontSize = 14;
    private const float SubtitleFontSize = 11;
    private const float NormalFontSize = 9;
    private const float SmallFontSize = 8;
    private const float TinyFontSize = 7;

    public PedidoVentaPdfDocument(PedidoVenta pedido)
    {
        _pedido = pedido ?? throw new ArgumentNullException(nameof(pedido));
        _moneda = pedido.Moneda?.ToUpperInvariant() switch
        {
            "USD" => TipoMoneda.USD,
            "EUR" => TipoMoneda.EUR,
            _ => TipoMoneda.CRC
        };
    }

    public DocumentMetadata GetMetadata() => new()
    {
        Title = $"Pedido de Venta - {_pedido.Numero}",
        Author = _pedido.Empresa?.RazonSocial ?? "Sistema de Facturación",
        Subject = "Pedido de Venta",
        Creator = "Sistema de Facturación Electrónica"
    };

    public void Compose(IDocumentContainer container)
    {
        container.Page(page =>
        {
            page.Size(PageSizes.Letter);
            page.Margin(30);
            page.DefaultTextStyle(x => x.FontSize(NormalFontSize).FontColor(TextColor));

            page.Header().Element(ComposePageHeader);
            page.Content().Element(ComposePageContent);
            page.Footer().Element(ComposeFooter);
        });
    }

    private void ComposePageHeader(IContainer container)
    {
        container.Column(col =>
        {
            col.Item().Row(row =>
            {
                row.RelativeItem(2).Column(left =>
                {
                    left.Item().Height(50).Width(150).Background(Colors.White)
                        .Border(1).BorderColor(BorderColor)
                        .AlignCenter().AlignMiddle()
                        .Text(_pedido.Empresa?.NombreComercial ?? _pedido.Empresa?.RazonSocial ?? "EMPRESA")
                        .FontSize(SmallFontSize).Bold().FontColor(PrimaryColor);

                    left.Item().Height(5);

                    left.Item().Text(_pedido.Empresa?.RazonSocial ?? "")
                        .FontSize(SubtitleFontSize).Bold().FontColor(TextColor);

                    if (!string.IsNullOrWhiteSpace(_pedido.Empresa?.NombreComercial) &&
                        _pedido.Empresa.NombreComercial != _pedido.Empresa.RazonSocial)
                    {
                        left.Item().Text(_pedido.Empresa.NombreComercial)
                            .FontSize(NormalFontSize).FontColor(LightTextColor);
                    }

                    left.Item().Text($"Cédula: {PdfFormatHelper.FormatIdentificacion(_pedido.Empresa?.NumeroIdentificacion, _pedido.Empresa?.TipoIdentificacion)}")
                        .FontSize(NormalFontSize).FontColor(TextColor);
                });

                row.ConstantItem(20);

                row.RelativeItem(1).Border(1).BorderColor(PrimaryColor).Background(HeaderBgColor).Padding(10).Column(right =>
                {
                    right.Item().AlignCenter().Text("PEDIDO DE VENTA")
                        .FontSize(TitleFontSize).Bold().FontColor(PrimaryColor);

                    right.Item().Height(5);

                    right.Item().AlignCenter().Text($"No. {_pedido.Numero}")
                        .FontSize(NormalFontSize).Bold().FontColor(TextColor);

                    right.Item().Height(3);

                    right.Item().AlignCenter().Text($"Fecha: {PdfFormatHelper.FormatFecha(_pedido.Fecha)}")
                        .FontSize(NormalFontSize).FontColor(TextColor);

                    right.Item().Height(3);

                    var estadoColor = _pedido.Estado switch
                    {
                        "APROBADO" => "#28a745",
                        "FACTURADO" => "#007bff",
                        "CANCELADO" => "#dc3545",
                        _ => "#6c757d"
                    };
                    right.Item().AlignCenter().Text(_pedido.Estado)
                        .FontSize(TinyFontSize).Bold().FontColor(estadoColor);
                });
            });
        });
    }

    private void ComposePageContent(IContainer container)
    {
        container.PaddingVertical(10).Column(col =>
        {
            col.Item().Row(row =>
            {
                row.RelativeItem().Element(ComposeEmisor);
                row.ConstantItem(10);
                row.RelativeItem().Element(ComposeReceptor);
            });

            col.Item().Height(10);
            col.Item().Element(ComposeCondiciones);
            col.Item().Height(10);
            col.Item().Element(ComposeLineItems);
            col.Item().Height(10);

            col.Item().Row(row =>
            {
                row.RelativeItem().Column(leftCol =>
                {
                    if (_pedido.FechaVencimiento.HasValue)
                    {
                        leftCol.Item().Border(1).BorderColor(BorderColor).Column(vigencia =>
                        {
                            vigencia.Item().Background(SecondaryColor).Padding(5)
                                .Text("VIGENCIA").FontSize(SmallFontSize).Bold().FontColor(Colors.White);
                            vigencia.Item().Padding(8)
                                .Text($"Válido hasta: {PdfFormatHelper.FormatFecha(_pedido.FechaVencimiento.Value)}")
                                .FontSize(NormalFontSize).FontColor(TextColor);
                        });
                        leftCol.Item().Height(5);
                    }

                    if (!string.IsNullOrWhiteSpace(_pedido.Observaciones))
                    {
                        leftCol.Item().Border(1).BorderColor(BorderColor).Column(notas =>
                        {
                            notas.Item().Background(HeaderBgColor).Padding(5)
                                .Text("OBSERVACIONES").FontSize(SmallFontSize).Bold().FontColor(TextColor);
                            notas.Item().Padding(8).Text(_pedido.Observaciones)
                                .FontSize(SmallFontSize).FontColor(TextColor);
                        });
                    }
                });

                row.ConstantItem(15);
                row.ConstantItem(220).Element(ComposeTotals);
            });
        });
    }

    private void ComposeEmisor(IContainer container)
    {
        container.Border(1).BorderColor(BorderColor).Column(col =>
        {
            col.Item().Background(PrimaryColor).Padding(5)
                .Text("EMISOR").FontSize(NormalFontSize).Bold().FontColor(Colors.White);

            col.Item().Padding(8).Column(content =>
            {
                content.Item().Text(_pedido.Empresa?.RazonSocial ?? "N/A")
                    .FontSize(NormalFontSize).Bold().FontColor(TextColor);

                if (!string.IsNullOrWhiteSpace(_pedido.Empresa?.NombreComercial))
                {
                    content.Item().Text($"Nombre Comercial: {_pedido.Empresa.NombreComercial}")
                        .FontSize(SmallFontSize).FontColor(LightTextColor);
                }

                content.Item().Text($"Identificación: {PdfFormatHelper.GetTipoIdentificacion(_pedido.Empresa?.TipoIdentificacion)} - {PdfFormatHelper.FormatIdentificacion(_pedido.Empresa?.NumeroIdentificacion, _pedido.Empresa?.TipoIdentificacion)}")
                    .FontSize(SmallFontSize).FontColor(TextColor);

                var email = _pedido.Empresa?.Emails?.FirstOrDefault()?.DireccionEmail;
                if (!string.IsNullOrWhiteSpace(email))
                {
                    content.Item().Text($"Email: {email}")
                        .FontSize(SmallFontSize).FontColor(TextColor);
                }

                var telefono = _pedido.Empresa?.Telefonos?.FirstOrDefault()?.NumeroTelefono;
                if (!string.IsNullOrWhiteSpace(telefono))
                {
                    content.Item().Text($"Teléfono: {telefono}")
                        .FontSize(SmallFontSize).FontColor(TextColor);
                }
            });
        });
    }

    private void ComposeReceptor(IContainer container)
    {
        container.Border(1).BorderColor(BorderColor).Column(col =>
        {
            col.Item().Background(PrimaryColor).Padding(5)
                .Text("CLIENTE").FontSize(NormalFontSize).Bold().FontColor(Colors.White);

            col.Item().Padding(8).Column(content =>
            {
                var cliente = _pedido.Cliente;
                content.Item().Text(cliente?.Nombre ?? "N/A")
                    .FontSize(NormalFontSize).Bold().FontColor(TextColor);

                if (!string.IsNullOrWhiteSpace(cliente?.NumeroIdentificacion))
                {
                    content.Item().Text($"Identificación: {PdfFormatHelper.GetTipoIdentificacion(cliente.TipoIdentificacion)} - {PdfFormatHelper.FormatIdentificacion(cliente.NumeroIdentificacion, cliente.TipoIdentificacion)}")
                        .FontSize(SmallFontSize).FontColor(TextColor);
                }

                if (!string.IsNullOrWhiteSpace(cliente?.EmailPrincipal))
                {
                    content.Item().Text($"Email: {cliente.EmailPrincipal}")
                        .FontSize(SmallFontSize).FontColor(TextColor);
                }

                if (!string.IsNullOrWhiteSpace(cliente?.TelefonoPrincipal))
                {
                    content.Item().Text($"Teléfono: {cliente.TelefonoPrincipal}")
                        .FontSize(SmallFontSize).FontColor(TextColor);
                }
            });
        });
    }

    private void ComposeCondiciones(IContainer container)
    {
        container.Border(1).BorderColor(BorderColor).Padding(8).Row(row =>
        {
            row.RelativeItem().Column(col =>
            {
                col.Item().Text("Condición de Venta:").FontSize(SmallFontSize).Bold().FontColor(LightTextColor);
                col.Item().Text(PdfFormatHelper.GetCondicionVenta(_pedido.CondicionVenta))
                    .FontSize(NormalFontSize).FontColor(TextColor);

                if (_pedido.PlazoCreditoDias.HasValue && _pedido.PlazoCreditoDias > 0)
                {
                    col.Item().Text($"Plazo: {_pedido.PlazoCreditoDias} días")
                        .FontSize(SmallFontSize).FontColor(LightTextColor);
                }
            });

            row.RelativeItem().Column(col =>
            {
                col.Item().Text("Medio de Pago:").FontSize(SmallFontSize).Bold().FontColor(LightTextColor);
                col.Item().Text(PdfFormatHelper.GetMedioPago(_pedido.MedioPago))
                    .FontSize(NormalFontSize).FontColor(TextColor);
            });

            row.RelativeItem().Column(col =>
            {
                col.Item().Text("Moneda:").FontSize(SmallFontSize).Bold().FontColor(LightTextColor);
                col.Item().Text(_pedido.Moneda ?? "CRC")
                    .FontSize(NormalFontSize).FontColor(TextColor);

                if (_pedido.TipoCambio.HasValue && _pedido.TipoCambio > 0 && _pedido.Moneda != "CRC")
                {
                    col.Item().Text($"T.C.: {_pedido.TipoCambio:N5}")
                        .FontSize(SmallFontSize).FontColor(LightTextColor);
                }
            });
        });
    }

    private void ComposeLineItems(IContainer container)
    {
        container.Column(col =>
        {
            col.Item().Border(1).BorderColor(BorderColor).Background(PrimaryColor).Padding(5).Row(row =>
            {
                row.ConstantItem(25).AlignCenter().Text("#").FontSize(SmallFontSize).Bold().FontColor(Colors.White);
                row.RelativeItem(3).Text("Descripción").FontSize(SmallFontSize).Bold().FontColor(Colors.White);
                row.ConstantItem(50).AlignCenter().Text("Cant.").FontSize(SmallFontSize).Bold().FontColor(Colors.White);
                row.ConstantItem(70).AlignRight().Text("Precio").FontSize(SmallFontSize).Bold().FontColor(Colors.White);
                row.ConstantItem(60).AlignRight().Text("Desc.").FontSize(SmallFontSize).Bold().FontColor(Colors.White);
                row.ConstantItem(60).AlignRight().Text("Imp.").FontSize(SmallFontSize).Bold().FontColor(Colors.White);
                row.ConstantItem(75).AlignRight().Text("Total").FontSize(SmallFontSize).Bold().FontColor(Colors.White);
            });

            var detalles = _pedido.Detalles?.OrderBy(d => d.NumeroLinea).ToList() ?? new List<PedidoVentaDetalle>();
            var isAlternate = false;

            foreach (var det in detalles)
            {
                var bgColor = isAlternate ? Color.FromHex(AlternateRowColor) : Colors.White;

                col.Item().Border(1).BorderColor(BorderColor).Background(bgColor).Padding(4).Row(row =>
                {
                    row.ConstantItem(25).AlignCenter().Text(det.NumeroLinea.ToString())
                        .FontSize(TinyFontSize).FontColor(TextColor);

                    row.RelativeItem(3).Text(PdfFormatHelper.TruncarTexto(det.Descripcion, 70))
                        .FontSize(TinyFontSize).FontColor(TextColor);

                    row.ConstantItem(50).AlignCenter().Text(PdfFormatHelper.FormatCantidad(det.Cantidad))
                        .FontSize(TinyFontSize).FontColor(TextColor);

                    row.ConstantItem(70).AlignRight().Text(PdfFormatHelper.FormatMoneda(det.PrecioUnitario, _moneda))
                        .FontSize(TinyFontSize).FontColor(TextColor);

                    row.ConstantItem(60).AlignRight().Text(det.MontoDescuento > 0 ? PdfFormatHelper.FormatMoneda(det.MontoDescuento, _moneda) : "-")
                        .FontSize(TinyFontSize).FontColor(TextColor);

                    row.ConstantItem(60).AlignRight().Text(det.MontoIVA > 0 ? PdfFormatHelper.FormatMoneda(det.MontoIVA, _moneda) : "-")
                        .FontSize(TinyFontSize).FontColor(TextColor);

                    row.ConstantItem(75).AlignRight().Text(PdfFormatHelper.FormatMoneda(det.TotalLinea, _moneda))
                        .FontSize(TinyFontSize).Bold().FontColor(TextColor);
                });

                isAlternate = !isAlternate;
            }
        });
    }

    private void ComposeTotals(IContainer container)
    {
        container.Border(1).BorderColor(PrimaryColor).Column(col =>
        {
            col.Item().Background(PrimaryColor).Padding(5)
                .Text("RESUMEN").FontSize(NormalFontSize).Bold().FontColor(Colors.White);

            col.Item().Padding(8).Column(content =>
            {
                ComposeTotalRow(content, "Subtotal:", _pedido.Subtotal);

                if (_pedido.TotalDescuentos > 0)
                    ComposeTotalRow(content, "Total Descuentos:", -_pedido.TotalDescuentos, true);

                if (_pedido.TotalImpuestos > 0)
                    ComposeTotalRow(content, "Total Impuestos:", _pedido.TotalImpuestos);

                content.Item().Height(5);
                content.Item().BorderBottom(2).BorderColor(PrimaryColor);
                content.Item().Height(5);

                content.Item().Row(row =>
                {
                    row.RelativeItem().AlignRight().Text("TOTAL:")
                        .FontSize(SubtitleFontSize).Bold().FontColor(PrimaryColor);
                    row.ConstantItem(10);
                    row.ConstantItem(100).AlignRight().Text(PdfFormatHelper.FormatMoneda(_pedido.Total, _moneda))
                        .FontSize(SubtitleFontSize).Bold().FontColor(PrimaryColor);
                });
            });
        });
    }

    private void ComposeTotalRow(ColumnDescriptor content, string label, decimal amount, bool isNegative = false)
    {
        content.Item().Row(row =>
        {
            row.RelativeItem().AlignRight().Text(label)
                .FontSize(SmallFontSize).FontColor(LightTextColor);
            row.ConstantItem(10);
            var displayAmount = isNegative
                ? $"({PdfFormatHelper.FormatMoneda(Math.Abs(amount), _moneda)})"
                : PdfFormatHelper.FormatMoneda(amount, _moneda);
            var color = isNegative ? "#dc3545" : TextColor;
            row.ConstantItem(100).AlignRight().Text(displayAmount)
                .FontSize(SmallFontSize).FontColor(Color.FromHex(color));
        });
    }

    private void ComposeFooter(IContainer container)
    {
        container.AlignCenter().Column(col =>
        {
            col.Item().BorderTop(1).BorderColor(BorderColor).PaddingTop(5);

            col.Item().PaddingBottom(3).AlignCenter()
                .Text("Este documento es un pedido de venta y no tiene validez fiscal.")
                .FontSize(TinyFontSize).Bold().FontColor(TextColor);

            col.Item().Row(row =>
            {
                row.RelativeItem().AlignLeft().Text(text =>
                {
                    text.Span("Generado: ").FontSize(TinyFontSize).FontColor(LightTextColor);
                    text.Span(PdfFormatHelper.FormatFechaHora(FechaCostaRicaHelper.Ahora)).FontSize(TinyFontSize).FontColor(LightTextColor);
                });

                row.RelativeItem().AlignCenter().Text(text =>
                {
                    text.Span("Pedido de Venta - Documento no fiscal").FontSize(TinyFontSize).FontColor(LightTextColor);
                });

                row.RelativeItem().AlignRight().Text(text =>
                {
                    text.Span("Página ").FontSize(TinyFontSize).FontColor(LightTextColor);
                    text.CurrentPageNumber().FontSize(TinyFontSize).FontColor(LightTextColor);
                    text.Span(" de ").FontSize(TinyFontSize).FontColor(LightTextColor);
                    text.TotalPages().FontSize(TinyFontSize).FontColor(LightTextColor);
                });
            });
        });
    }
}
