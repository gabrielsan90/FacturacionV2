    ...
    public async Task<IActionResult> OnPostSaveAsync()
    {
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                if (isNew)
                {
                    // La API retorna { success: true, empresaId: "..." }
                    try
                    {
                        var responseContent = await response.Content.ReadAsStringAsync();
                        var jsonDoc = JsonDocument.Parse(responseContent);
                        Guid? empresaId = null;
                        if (jsonDoc.RootElement.TryGetProperty("empresaId", out var empIdProp))
                        {
                            empresaId = empIdProp.GetGuid();
                        }

                        _logger.LogInformation("Empresa creada exitosamente: {EmpresaId}", empresaId);
                    ...
                    {
                        _logger.LogError(ex, "Error leyendo respuesta de empresa creada");
                        return new JsonResult(new
                        {
                            success = true,
                            message = "Empresa creada exitosamente"
